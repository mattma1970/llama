...
class AudioConnection(ABC):
	...
	def __init__(self,conn,audio_settings={}, processing_func:Callable =None, **kwargs):
		self.conn = conn  # the stream or webrtc connection..
		self.audio_settings=audio_settings #audio settings e.g. sample rate
		self.processing_func =processing_func # Callable used to convert frames to format needed by consuming STT. Callable must only require a single parameter whcih must be the list of audioframes.
	...
	@abstractmethod
	def processed_frames(self, processing_func: Callable = None):
...
class WebRTCAudioSteam(AudioConnection):
	...
	'''
	def __init__(self, conn: webrtc_streamer = None, audio_settings: dict = {}, processing_func: Callable = None, **kwargs):
		super().__init__(conn ,audio_settings)
		if 'max_frames' in kwargs:
			self.MAXFRAMES= kwargs['max_frames']
		else:
			self.MAXFRAMES=99 
		...
		if conn is None:
			self.conn, self.audio_settings = setup_webRTC(use_ice_server=('local' in kwargs), timeout_sec=60) # This is blocking
		...
		@process_func.setter
		def process_func(self,func: Callable):
			if func is None:
				#provide the default base64 utf-8 encoding of audio used by assembly.ai. This can be overidden by passing in a callable(data)
				def inner(self, data: av.AudioFrames):
					'''
	...
	def ready(self):
		...
		if self.conn is None:
			...
			return self.conn.state.playing and self.conn.audio_reciever
	
	def read_frames(self, timeout: float, **kwargs):
		'''Reads the audio queue and truncates it if queue was too long for STT API. '''
		try:
			if self.conn.audio_receiver:      
				try:
					...
					while len(self.frames)<self.audio_settings['required_frames']:
						audio_frames.extend(self.conn.audio_receiver.get_frames(timeout=timeout))
				except queue.Empty:
			...
			if len(audio_frames) > self.MAX_FRAMES:
				audio_frames=audio_frames[-self.MAX_FRAMES:]
...
class PyAudioStream(AudioConnection):
	def __init__(self,conn=None, audio_settings={}, processing_func:Callable=None):
		import pyaudio
		self.p=pyaudio.PyAudio()

		super().__init(conn,audio_settings, processing_func)
		...
		def process_func(self,func: Callable):
			if func is None:
				...
				self._process_func = processing_func

		#unlike webRTC audio settings are perscriptive.
	...
	def read_frames(self, timeout=100):
		if self.stream:
			try:
	...
	def processed_frames(self):
			if self.frames_available:
				return self.processing_func(self.data)
			else:
				return None
